...
| `huo15-img-prompt` | `huo15-img-prompt` | v2.3.0 | 火一五文生图提示词六件套（88 预设 + 混合预设 + 五锁一致性 + 角色设定图 + 视频提示词 9 模型 + 关键帧 + 参考图反解 + ComfyUI/SD-WebUI/DALL-E 直出 + Claude API 智能润色 + 平台合规重写） | 火一五文生图提示词、文生图、文生视频、提示词增强、生成图片、生成视频、角色一致、系列图、混合风格、参考图反解、Claude 润色、平台合规、ComfyUI 直出、Sora、可灵、Runway、敦煌风、原神 |
