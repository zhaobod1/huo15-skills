# 小程序 Starter — 火一五前端设计技能 v4.2

> 微信 + 支付宝 + 抖音 **三端** starter。**浏览器无法直接渲染** wxml / axml / ttml，必须用各家 IDE。
...
| `wxml` 文件名 | `ttml` |
| `wxss` 文件名 | `ttss` |
| `bindtap` / `bindinput` | **保留**（抖音兼容微信 bind 风格） |
| `<page-meta>` | **支持**，但建议用 `app.json window` 配置代替（更稳） |

### 步骤 3：复制到 `alipay/` 改前缀（机械替换，5 分钟）
...
| `wxml` 文件名 | `axml` |
| `wxss` 文件名 | `acss` |
| `<page-meta>` | **不支持**，改用 `my.setNavigationBar` API（写在 onLoad） |
| `enhanced="true"` 等微信特有属性 | 删掉 |
| `app.json` tabBar `color`/`text` | `textColor`/`name` |

### 步骤 4：三端 ICON / 图片资源建议放 CDN（本地用 base64 或纯色块占位）

## 反 AI Slop 红线（v2.2 引入小程序专属 #14 / #15，v4.2 扩展 UI 库列表）

| # | 禁用项 | 替代方案 |
|---|---|---|
| 14 | 直接套 **WeUI / Vant Weapp / TDesign-Mini / Lin-UI / TTUI / Tt-Mini-UI** 默认皮 | 自定义 token，至少改 5 个变量再用 |
...
三端 starter 共用 ORGANIC 流派暖色调（米白 + 土橙 + 暖卡片），主题"手作商品列表"。`hero` + `chips` + 商品 `grid` 三段式，避开千篇一律的"banner + features + cta"。
...
- 多端同步迭代速查见本文 §三端同步迭代姿势

## 三端真机扫码后必查清单

- [ ] 顶部胶囊（微信 / 支付宝 / 抖音三端形态略不同）不遮内容
