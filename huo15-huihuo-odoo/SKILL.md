...
description: >-
  ...
  (www.huo15.com, db=huo15) through its API. Covers three apps: 待办/To-do
  (create, list, complete personal tasks with title, deadline, stage, priority),
  项目/Project (edit projects, manage tasks, stages, assignees, hours), and
  工时单/Timesheets (per-employee / per-project / per-month hour reports and
  filtered statistics). Also use when the user mentions odoo, 辉火云企业套件,
  公司系统的待办/项目/工时单, www.huo15.com/odoo/to-do, /odoo/project,
  /odoo/timesheets, or asks to log in to and save credentials for the company
...
三大能力：**待办**（/odoo/to-do）、**项目**（/odoo/project）、**工时单**（/odoo/timesheets）。

> API 知识沉淀（出问题先查这里）：
> [references/odoo-todo-api.md](references/odoo-todo-api.md) ·
> [references/odoo-project-api.md](references/odoo-project-api.md) ·
> [references/odoo-timesheet-api.md](references/odoo-timesheet-api.md)
...
| 想做 | 命令 |
|---|---|
| 配登录 | `printf '%s' "$PWD" \| python3 scripts/login.py set --login X` |
